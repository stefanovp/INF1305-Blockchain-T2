...
set -ev
for CONTAINER in $(docker ps -f label=fabric-environment-name="votingNetwork" -q -a); do
    docker rm -f ${CONTAINER}
done
for VOLUME in $(docker volume ls -f label=fabric-environment-name="votingNetwork" -q); do
